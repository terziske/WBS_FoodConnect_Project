...
export default {
  ...
  created() {
    this.getMainDishRecipes();
  },
  methods: {
    async getMainDishRecipes() {
      await axios
        .get("http://localhost:8080/api/courses/search?course=Main%20course")
        .then((response) => {
    ...
    getRecipeLink(recipe){
      return "/recipes/" + encodeURIComponent(recipe.replace('_', '%20')) + "?category=dish";
    },
    async searchRecipe(){
      this.loading = true;
      await axios
        .get("http://localhost:8080/api/recipes/q?name=" + this.searchInput + "&category=dish")
        .then((response) => {
