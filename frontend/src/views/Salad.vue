...
export default {
  ...
  created() {
    this.getSaladRecipes();
  },
  methods: {
    async getSaladRecipes() {
      await axios
        .get("http://localhost:8080/api/courses/search?course=Salad")
        .then((response) => {
    ...
    getRecipeLink(recipe){
      return "/recipes/" + encodeURIComponent(recipe.replace('_', '%20')) + "?category=salad";
    },
    async searchRecipe(){
      this.loading = true;
      await axios
        .get("http://localhost:8080/api/recipes/q?name=" + this.searchInput + "&category=salad")
        .then((response) => {
